...
const input = new InputPandler();
	...
	class Particle {
		...
		update(){
			...
			if (this.y > this.game.height - this.bottomBounceBoundary && this.bounced < 7){
				this.bounced++;
				this.speedY *= -0.77;
			} 
		}
	...
	class Enemy {
		constructor(game){
			...
			this.speedX = Math.random() * -9.5 - 2.5;
			this.markedForDeletion = false;
	...
	//		this.background.layer5.draw(context);
		...
		addEnemy(){
			...
			else if (randomize > 0.5) this.enemies.push(new LuckyFish(this));
